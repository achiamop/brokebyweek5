...
def render_favorite_model_tab():
    """Render the favorite betting model tab"""
    st.header("⭐ Simple Moneyline Favorite Model")
    st.markdown("**Simple strategy: Always bet the moneyline favorite**")
    ...
    st.success(f"Loaded {len(df)} games from 2020-2024 seasons")
    
    st.sidebar.header("Favorite Model Configuration")
    
    st.sidebar.subheader("Bet Settings")
    bet_size = st.sidebar.slider(
        "Bet Size ($)",
        min_value=25,
        max_value=500,
        value=100,
    ...
    st.sidebar.subheader("Favorite Odds Filter")
    fav_odds_min = st.sidebar.slider("Min Favorite Odds (absolute)", 100, 500, 100, step=10, key="fav_odds_min")
    fav_odds_max = st.sidebar.slider("Max Favorite Odds (absolute)", 100, 500, 400, step=10, key="fav_odds_max")
    
    st.sidebar.subheader("Analysis Period")
    available_seasons = sorted(df['season'].unique())
    fav_seasons = st.sidebar.multiselect("Seasons", available_seasons, default=available_seasons[-3:], key="fav_seasons")
    fav_week_range = st.sidebar.slider("Week Range", 1, 18, (1, 18), key="fav_weeks")
    
    if st.sidebar.button("🔄 Run Favorite Analysis", type="primary", key="run_fav_btn"):
        if not fav_seasons:
            st.warning("Please select at least one season.")
            return
            
        with st.spinner("Running favorite betting analysis..."):
            bets_df, summary = analyze_favorite_betting(
                df, fav_odds_min, fav_odds_max, fav_seasons, fav_week_range, bet_size
            )
            
            st.session_state['fav_bets_df'] = bets_df
            st.session_state['fav_summary'] = summary
            st.session_state['fav_analysis_complete'] = True
    
    if st.session_state.get('fav_analysis_complete', False):
        bets_df = st.session_state['fav_bets_df']
        summary = st.session_state['fav_summary']
        
        if len(bets_df) == 0:
            st.warning("No games found with current settings. Try adjusting your filters.")
            return
        
        st.subheader("📊 Favorite Betting Results")
